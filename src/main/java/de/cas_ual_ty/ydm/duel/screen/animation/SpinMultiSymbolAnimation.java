...
public class SpinMultiSymbolAnimation extends Animation
{
	// TODO: Multiple Symbols that spin, spawning in locations randomly near the center symbol.
	public float centerPosX;
    public float centerPosY;
    public int size;
    public int endSize;
    
    public SpinMultiSymbolAnimation(float centerPosX, float centerPosY, int size, int endSize)
    {
    ...
    public void render(MatrixStack ms, int mouseX, int mouseY, float partialTicks)
    {
        ...
        // I'm pretty sure this is NOT the way to do this
        ms.translate(centerPosX + ((int) Math.random() * 65) - ((int) Math.random() * 65) , centerPosY + ((int) Math.random() * 65) - ((int) Math.random() * 65), 0);
        YdmBlitUtil.fullBlit(ms, -halfSize, -halfSize, size, size);
